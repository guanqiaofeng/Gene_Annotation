...
open OUT, ">Poplar_Fst_gene_anno_output.txt";
print OUT "Locus\tGene\tAt_ortholog\tAnnotation\tFst";

# P. trichocarpa pacbio v1.1 gene blast best hit in Arabidopsis Araport11 #
%PuAt = ();
open FH, "<Ptri_v1.1_to_Ara11.txt";
while (<FH>)
{
    if (/^(\S+)\s+(\S+)/)
    {
...
%genelist = ();
open FH, "<Ptrichocarpavar145v1.1b.gene.gff3";
while (<FH>)
{
